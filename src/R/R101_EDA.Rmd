...
```{r}
cor(aset$AGE_AT_ALS1DX,aset$DAYS_ALS1DX_TO_DEATH,method="pearson")
```

*Spearman Correlation*
```{r}
cor(aset$AGE_AT_ALS1DX,aset$DAYS_ALS1DX_TO_DEATH,method="spearman")
```
...
```{r}
ggplot(aset, aes(x=DAYS_ALS1DX_TO_DEATH,y=AGE_AT_ALS1DX))+
  geom_point()+
...
```{r}
fit_aov<-aov(AGE_AT_ALS1DX ~ RACE, data=aset)
summary(fit_aov)
```
...
```{r bmi_boxplot}
#vertical boxplot
ggplot(aset,aes(y=AGE_AT_ALS1DX,fill=RACE))+
  geom_boxplot()
```

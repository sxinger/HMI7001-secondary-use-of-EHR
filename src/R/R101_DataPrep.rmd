...
pt_tbl_only_agegrp<-pt_tbl_add_agegrp %>%
  select(PATID,age_group)

ggplot(pt_tbl_only_agegrp,aes(x=age_group))+
  geom_bar(stat="count")
```
...
```{r}
##=====================================================
# "str" function is a handy function for quick check of
...
Let's replace the categorical "RACE" column by the numerical representation based on one-hot-encoding
```{r}
...
```{r}
saveRDS(final_set,file="pt_tbl<-readRDS("C:/Users/Administrator/Documents/classHMI7001/data/final_set.rda")")
```
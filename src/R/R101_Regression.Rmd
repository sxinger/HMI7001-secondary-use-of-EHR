...
```{r}
fit1<-glm(RILUZ_DUR ~ AGE_AT_ALS1DX, data=aset)
...
* Homoscedasticity (equal variance): residual vs. preditted plot, or actual vs. predicted plot
...
```{r bmi_cat_sa_glm}
fit3<-glm(MORT_1YR ~ AGE_AT_ALS1DX, 
          data=aset2, 
          family='binomial')
summary(fit3)
```
...
resid_dat<-resid_dat %>% 
  mutate(MORT_1YR_hat=case_when(predicted>=0.2 ~ 1,
                               TRUE ~ 0))
confusion_matrix<-table(resid_dat$MORT_1YR,resid_dat$actual,
